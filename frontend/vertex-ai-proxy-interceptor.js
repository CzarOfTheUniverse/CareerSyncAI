...
(function() {
  ...
  // The signed-in user's Google access token. The backend verifies it against
  // its allow-list when auth is enforced. Read fresh each call so a re-login is
  // picked up without reloading the page.
  function getUserToken() {
    try {
      return localStorage.getItem('career_sync_token') || '';
    } catch (e) {
      return '';
    }
  }
  ...
  window.WebSocket = function(url, protocols) {
    ...
    if (inputUrl && isValidUrl(inputUrl)) {
      ...
      const authParam = userToken ? `&access_token=${encodeURIComponent(userToken)}` : '';
      const proxyUrl = `${protocol}//${host}/ws-proxy?target=${targetUrl}${authParam}`;
      ...
      try {
        ...
        const proxyFetchOptions = {
          ...
          headers: {
            ...
            'X-App-Proxy': 'sJXKVn8dmTmteik531RngQrw3ny8zL0S',
            // The signed-in user's Google token, verified against the allow-list
            // when the backend has ALLOWED_EMAILS / ALLOWED_HOSTED_DOMAIN set.
            'X-User-Access-Token': getUserToken(),
          },
